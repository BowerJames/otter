...
class ProviderConfig(BaseModel):
    """Registry entry for a provider: identity + env-key.

    Built-in entries are seeded by :func:`register_built_ins`; additional
    entries are added via :func:`register_provider`. Only ``env_key`` is read
    here (by :func:`get_env_api_key`). Dispatch routes on the caller's
    :class:`~otter_ai_core.ProviderModelOption.api`, so the per-provider
    api is not stored on this registry entry.
    """
