...
:mod:`otter_ai_core.bidirectional_stream` for live, bidirectional model APIs
(Realtime / Responses-style). It fixes the connection's generic event types
to concrete unions:

* :data:`ModelConnection` — ``BidirectionalStream[ClientEvent, ServerEvent]``.
* :data:`ModelConnectionFn` — ``BidirectionalStreamFn[ClientEvent, ServerEvent]``,
  the options-bound producer
  (:data:`BidirectionalStreamFn` is
...
API registry, or dispatch** — only the generic bidirectional-stream runtime
(in :mod:`otter_ai_core.bidirectional_stream`) and these typed aliases / event
unions.
"""
