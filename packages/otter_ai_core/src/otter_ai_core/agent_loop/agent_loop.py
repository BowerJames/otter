...
from otter_ai_core.model_controller import ModelController, create_model_controller_stream
from otter_ai_core.model_connection import ServerContextEventType, ResponseDone, AddUserMessage
...
class AgentLoop:
    ...
    async def _run_inner_loop(self, messages: list[UserMessage]) -> None:
        while not self._steering_queue.empty():
            ...
            response = await _generate(self._controller, messages)
...
async def _generate(controller: ModelController, messages: list[UserMessage]) -> ResponseDone:
    controller.generate(
        [AddUserMessage(message=message) for message in messages]
    )
