...
class AgentSession:
    ...
    async def __aenter__(self) -> Self:
        ...
        model_entered = False
        model: EnterableModel | None = None
        try:
            ...
            if model_entered:
                ...
                await model.__aexit__(type(error), error, traceback)
            if manager_entered:
                await self._session_manager.__aexit__(type(error), error, traceback)
            raise
        self._model = model
    ...
    async def __aexit__(
        ...
        exits the model and closes the session manager. Never suppresses
        ...
        if self._model is not None:
            await self._model.__aexit__(exc_type, exc_value, traceback)
        await self._session_manager.__aexit__(exc_type, exc_value, traceback)
        self._state = _SessionState.CLOSED
    ...
    def is_idle(self) -> bool:
        ...
        if self._poisoned:
            return False
        return self._run_task is None or self._run_task.done()
    ...
    def _require_agent(self) -> Agent:
        if self._agent is None:
            raise RuntimeError("the session has no agent; __aenter__ must complete first")
        return self._agent
